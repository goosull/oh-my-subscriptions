...
test("OMS order wins and blocked, stale, unknown and mismatched accounts are excluded", () => {
  ...
  const codex = { name: "codex", vendor: "codex", available: true };
  const claude = { name: "claude", vendor: "claude", available: true };
  expect(approved({ accounts: [codex, claude] }, routes).map(r => r.account)).toEqual(["codex", "claude"]);
